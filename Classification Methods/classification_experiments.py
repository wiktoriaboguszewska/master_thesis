...
from MLLS import em
from TC import threshold_calibration
...
def run_em(X_tr, y_tr, X_te, clf_name):
    q_x, _ = em(X_tr, y_tr, X_te, clf_name=clf_name, epochs=200, q_init=0.5, tol=1e-4)
    return (q_x[:,1] >= 0.5).astype(int)


def run_thresh(X_tr, y_tr, X_te, clf_name):
    q_x, _, _ = threshold_calibration(X_tr, y_tr, X_te, clf_name=clf_name)
    return (q_x[:,1] >= 0.5).astype(int)
